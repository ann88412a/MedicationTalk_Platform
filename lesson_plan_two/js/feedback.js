...
function feedback(){
    ...
    if (!document.getElementById('check5').checked){
        ...
        if (pill_detect['Cardilo'] == 0 && pill_detect['Concor'] == 0){ 
            ...
            callOpenAI("不給藥", "不給藥", "藥物錯誤", userAnswer, "核對不僅是藥袋名稱，還要注意藥袋內的藥名，Concor 和Cardio乍看英文字很像，因此需要小心辨識!").then(apiResponse => {
            ...
            // q_time = q_time + 1;
            const userAnswer = document.getElementById('Concor 5mg/tab r no').value;
            callOpenAI("不給藥", "給藥", "藥物錯誤", userAnswer, "核對不僅是藥袋名稱，還要注意藥袋內的藥名，Concor 和Cardio乍看英文字很像，因此需要小心辨識!").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    img2.src = "pic/wrong_w.png";
                    r2r = apiResponse.replace(/^[^\u4e00-\u9fa5]+/, '');
                    correctness.push(20);     
        ...
        callOpenAI2("核對不僅是藥袋名稱，還要注意藥袋內的藥名，Concor 和Cardio乍看英文字很像，因此需要小心辨識!").then(apiResponse => {
            if (apiResponse && typeof apiResponse === 'string') {
    ...
    if (!document.getElementById('check4').checked){
        ...
        if (pill_detect['Isoptin 40mg/tab'] == 0){
            ...
            callOpenAI("不給藥", "不給藥", "劑量錯誤", userAnswer, "同一種藥物會有不同劑量").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    if (apiResponse[0] == "1") {
                      score += 1;
                      img3.src = "pic/ok_w.png";
            ...
            // q_time = q_time + 1;
            const userAnswer = document.getElementById('Isoptin 40 mg/tab r no').value;
            callOpenAI("不給藥", "給藥", "劑量錯誤", userAnswer, "同一種藥物會有不同劑量").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    img3.src = "pic/wrong_w.png";
                    r3r = apiResponse.replace(/^[^\u4e00-\u9fa5]+/, '');
                    correctness.push(30);
        ...
        callOpenAI2("同一種藥物會有不同劑量").then(apiResponse => {
            if (apiResponse && typeof apiResponse === 'string') {
    ...
    if (!document.getElementById('check1').checked){
        ...
        callOpenAI("不給藥", "不給藥", "途徑錯誤", userAnswer, "Amikacin建議稀釋由靜脈滴注給IV drip100-200ml，且滴注30-60分鐘，抗生素大多需要稀釋，並且滴注給予").then(apiResponse => {
            if (apiResponse && typeof apiResponse === 'string') {
                if (apiResponse[0] == "1") {
                    score += 1;
        ...
        cognition.push(40);
        img4.src="pic/wrong_w.png";
        // r4 = '您給 Amikacin 250mg/vail 的理由：' + document.getElementById('Amikacin 250mg/vail r').value + "<br> -> 答錯原因:「三讀五對」認知錯誤" +
        ...
        callOpenAI2("Amikacin建議稀釋由靜脈滴注給IV drip100-200ml，且滴注30-60分鐘，抗生素大多需要稀釋，並且滴注給予").then(apiResponse => {
            if (apiResponse && typeof apiResponse === 'string') {
    ...
    if (!document.getElementById('check2').checked){
        ...
        callOpenAI2("病人服用 Metformin ，要注射顯影劑的電腦斷層，其檢查前後需停用 48 小時。由於注射顯影劑會暫時加重腎臟過濾雜質的工作負擔，而Metformin的代謝廢物必須經由腎臟排出，一旦藥物的代謝廢物排不出去，可能會引起身體內乳酸中毒").then(apiResponse => {
            if (apiResponse && typeof apiResponse === 'string') {
    ...
    if (document.getElementById('check6').checked){
        ...
        if (pill_detect['Nexium'] == 1){
            ...
            callOpenAI("給藥", "給藥", "胃潰瘍，使用氫離子幫浦抑制劑PPI抑制胃酸分泌", userAnswer, "給藥前，必須先確定患者臨床上有服用該藥物的適應症，並且執行給藥醫囑").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    if (apiResponse[0] == "1") {
                      score += 1;
                      img8.src = "pic/ok_w.png";
            ...
            // q_time = q_time + 1;
            const userAnswer = document.getElementById('Nexium 40mg/tab r').value;
            callOpenAI("給藥", "不給藥", "胃潰瘍，使用氫離子幫浦抑制劑PPI抑制胃酸分泌", userAnswer, "給藥前，必須先確定患者臨床上有服用該藥物的適應症，並且執行給藥醫囑").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    img8.src = "pic/wrong_w.png";
                    r8r = apiResponse.replace(/^[^\u4e00-\u9fa5]+/, '');
                    correctness.push(80);
    ...
    if (document.getElementById('check7').checked){
        ...
        if (pill_detect['Lipitor'] == 1){ 
            ...
            callOpenAI("給藥", "給藥", "高血脂，服用Lipitor可降低血中過高之膽固醇及血脂，減少心血管疾病及中風的發作", userAnswer, "給藥前，必須先確定患者臨床上有服用該藥物的適應症，並且執行給藥醫囑").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    if (apiResponse[0] == "1") {
                      score += 1;
                      img9.src = "pic/ok_w.png";
            ...
            // q_time = q_time + 1;
            const userAnswer = document.getElementById('Lipitor 20mg/tab r').value;
            callOpenAI("給藥", "不給藥", "高血脂，服用Lipitor可降低血中過高之膽固醇及血脂，減少心血管疾病及中風的發作", userAnswer, "給藥前，必須先確定患者臨床上有服用該藥物的適應症，並且執行給藥醫囑").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    img9.src = "pic/wrong_w.png";
                    r9r = apiResponse.replace(/^[^\u4e00-\u9fa5]+/, '');
                    correctness.push(90);
    ...
    if (!document.getElementById('check8').checked){
        ...
        if (pill_detect['Sandimmunneoral'] == 0){
            ...
            callOpenAI("不給藥", "不給藥", "藥物交互作用", userAnswer, "病人有嚴重乾癬，有服用 Sandimmun neoral的適應症，它與 Lipitor (atorvastatin) 合用，會有藥物交互作用 DDI，這兩種藥物合併使用可能會增加Lipitor在血液中的濃度，因此可能增加其副作用的風險，例如肌肉疼痛或肝臟問題。藥物-藥物交互作用(drug-drug interaction, DDI)，A藥與B藥一起使用，其相互作用後可能會造成藥效作用延遲、減少或增強任一藥物的吸收而引起不良反應 ").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    if (apiResponse[0] == "1") {
                      score += 1;
                      img10.src = "pic/ok_w.png";
            ...
            // q_time = q_time + 1;
            const userAnswer = document.getElementById('Sandimmun neoral 100mg/tab r no').value;
            callOpenAI("不給藥", "給藥", "藥物交互作用", userAnswer, "病人有嚴重乾癬，有服用 Sandimmun neoral的適應症，它與 Lipitor (atorvastatin) 合用，會有藥物交互作用 DDI，這兩種藥物合併使用可能會增加Lipitor在血液中的濃度，因此可能增加其副作用的風險，例如肌肉疼痛或肝臟問題。藥物-藥物交互作用(drug-drug interaction, DDI)，A藥與B藥一起使用，其相互作用後可能會造成藥效作用延遲、減少或增強任一藥物的吸收而引起不良反應 ").then(apiResponse => {
                if (apiResponse && typeof apiResponse === 'string') {
                    img10.src = "pic/wrong_w.png";
                    r10r = apiResponse.replace(/^[^\u4e00-\u9fa5]+/, '');
                    correctness.push(100);
        ...
        callOpenAI2("病人有嚴重乾癬，有服用 Sandimmun neoral的適應症，它與 Lipitor (atorvastatin) 合用，會有藥物交互作用 DDI，這兩種藥物合併使用可能會增加Lipitor在血液中的濃度，因此可能增加其副作用的風險，例如肌肉疼痛或肝臟問題。藥物-藥物交互作用(drug-drug interaction, DDI)，A藥與B藥一起使用，其相互作用後可能會造成藥效作用延遲、減少或增強任一藥物的吸收而引起不良反應 ").then(apiResponse => {
            if (apiResponse && typeof apiResponse === 'string') {
