...
radio2.addEventListener('change', function (e) {
    ...
    $('.bar_no')[0].innerText = '已更換病人資訊為: 李雅蘭(女) 出生年月日: 28/12/11 病歷號: 04154212 身分證字號： F210873560';
}, false);
...
for (let i=0; i<10; i++){
    ...
    listener_list.push(check_list[i].addEventListener('change', function(e){
        ...
        if (i==0){ // "Amikacin 250mg/vail"
            create_json('Amikacin 250mg/vail', check_list[i].checked);
        }
        else if(i==8){ // "Keto 30mg/amp"
            create_json('Keto 30mg/amp', check_list[i].checked);
        }
        ;
...
function check_bt(f){
    if (f === 'barcode'){
        ...
        dan.push('LessonPlan-I', [client_uid, machine_ID, 2]);

        output_patient_barcode_bt = output_patient_barcode_bt + 1;
    }
    else if(f === 'medicine_info'){
        dan.push('BarcodeScanCmd-I', [client_uid, machine_ID, 'syringe', true]);
    }
    else if(f === 'pill'){
